"""GrokAgent — the real LLM implementation of the `Agent` protocol.

Uses xAI's OpenAI-compatible endpoint with JSON-schema structured outputs, so
the decision object is guaranteed to match the shape the workflow expects and
we never hand-parse loose JSON.

Two models, deliberately:
  * main agent  -> a Grok 4 family model (structured outputs + tools)
  * classifier  -> a cheaper/faster model; the spec calls for something
                   "lightweight" to gate whether the main agent wakes.

Set AGENT_MODE=grok and XAI_API_KEY to enable. If a call fails, the exception
propagates out of the activity and Temporal retries it with backoff — that is
how free-tier rate limits are absorbed without any bespoke retry code.
"""
...
MAIN_MODEL = os.environ.get("XAI_MODEL", "grok-4.5")
CLASSIFIER_MODEL = os.environ.get("XAI_CLASSIFIER_MODEL", "grok-4.1-fast")
TIMEOUT = httpx.Timeout(60.0, connect=10.0)

# --------------------------------------------------------------------------
# JSON schemas. `strict` + `additionalProperties: false` is what makes the
# response shape a guarantee rather than a hope.
# --------------------------------------------------------------------------
...
class GrokAgent:
    """Calls xAI. Falls back to nothing — errors propagate so Temporal retries."""
    ...
    def __init__(self) -> None:
        ...
        if not self.api_key:
            raise RuntimeError(
                "AGENT_MODE=grok but XAI_API_KEY is not set. "
